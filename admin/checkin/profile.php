<?php
    $sql_data = $conndb->query(" SELECT m.id , m.m_card , m.fname , m.package , t.date ,m.image , 
    t.time , m.sta_date , m.exp_date ,m.comment,
    t.time_id , m.group , m.expired , m.type_training , m.type_fighter , m.sponsored ,
     m.status , m.emergency , m.dropin , p.product_name , m.status_code 
    FROM tb_time AS t
...
?>
    <h2 style="font-weight: 700; margin-top: 15px; text-align: center; color:#000;"> <?= strtoupper($result[0]['fname']); ?> </h2>    

    <?php if ($result[0]['status_code'] == 1 ) { ?>
        <?php echo $result[0]['status_code'] ?> 
        <img src=" <?= '../dist/img/'.$result[0]['image'] ?>" width="70%"  class="mt-3 img" >
        <!-- package -->
    ...
    <?php } elseif ($result[0]['status_code'] == 4 ) { ?>
    ...
    <?php } elseif ($result[0]['status_code'] == 2 ) { ?>
        <img src=" <?= '../memberimg/img/'.$result[0]['image'] ?>" width="50%"  class="mt-3 img" >
        <!-- package -->
        <div class="input-group mb-1 mt-3 col-9 mx-auto">
            <span class="input-group-text mr-1 bg-info" style=" text-transform: uppercase;font-weight:600;">Package name</span>
            <input type="text" class="form-control" value="<?= $result[0]['package'] ?>" >
        </div>
        <!-- sta_date -->
        <div class="input-group mb-1 col-9 mx-auto">
            <span class="input-group-text mr-1 bg-info" style=" text-transform: uppercase;font-weight:600;">Start Train</span>
    ...
    <?php } elseif ($result[0]['status_code'] == 3 ) { ?>

        <img src=" <?= '../fighterimg/img/'.$result[0]['image'] ?>" width="70%" class="img img-fluid">
