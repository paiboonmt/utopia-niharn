...
if (isset($_POST['ref_m_card'])) {
    ...
    if (checkNumber($conndb, $m_card) == 1) { // เช็คหมายเลขบัตร
        if (exp_date($conndb, $m_card) >= 0) { // เช็ควันหมดอายุ
            echo checkProduct($conndb, $m_card); // เช็คสินค้า
        } else {
            $_SESSION['expiry'] = true; // หมดอายุ
            header('location:checkin.php'); // ส่งไปหน้า checkin
            $conndb = null; // สำหรับหมดอายุ 
        }
    } else if (checkNumber($conndb, $m_card) == 2) { // เช็คหมายเลขบัตร
        if (exp_date($conndb, $m_card) >= 0) { // เช็ควันหมดอายุ
            echo checkProduct($conndb, $m_card); // เช็คสินค้า
        } else {
            $_SESSION['expiry'] = true; // หมดอายุ
...
function checkProductType($conndb, $m_card)
{
    ...
    if ($result) {
        return $result['product_type']; // Return the product type
    } else {
