  ...
  <div class="content-wrapper">
   <nav class="nav_menu">
      ...
      <ul>
        ...
        <li><a href="report_all_time.php" class="active" > report all time </a></li>
            ...
            <table id="example1" class="table  table-hover">
               ...
               <tbody>
                   <?php 
                      $count = 1 ;
                      require_once '../includes/connection.php';
                      $stmt = $conndb->prepare(" SELECT m.id , m.m_card , m.fname ,
                        m.nationalty, m.package ,t.date,m.accom,m.sta_date,m.exp_date,m.payment,m.vaccine,m.sponsored
                      FROM tb_time AS t
                      INNER JOIN member AS m ON t.ref_m_card = m.m_card 
                      ORDER BY t.time DESC LIMIT 15000 ");
                      $stmt->execute();
                   ...
                   <tr>
                       ...
                       <td> <?php echo $row['package'].$row['sponsored']?> </td>
                       <td> <?php echo date('d/m/Y H:i:s',strtotime($row['date'])) ?>
                       </td>
                       <td> <?php echo $row['accom'] ?> </td>
